...
export function TariffTables() {
  ...
  const getMainSubtitle = () => {
    switch (language) {
      ...
      default:
        return "Стоимость услу�� курьерской службы в Республике Узбекистан (без учёта НДС)";
    }
  };
    ...
    switch (language) {
      case "ru":
        return {
          ...
          description: "От общей ��уммы принятых наложенных платежей.",
        };
      case "uz":
        return {
    ...
    switch (language) {
      case "ru":
        return {
          ...
          description:
            "Дополнительный платеж, уплачиваемый Заказчиком Исполнителю при оглашении стоимости почтов��го отправления. Расчет размера дополнительного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x объявленная стоимость (сумма).",
        };
      case "uz":
        return {
      ...
      default:
        return {
          ...
          cost: "Стоимость: 1% (без учета НДС 12%)",
          description:
            "Допо��нительный платеж, уплачиваемый Заказчи�����м Исполнителю при оглашении стоимости почтового отправления. Расчет размера доп��лн��тельного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x объявленная стоимость (сумма).",
        };
    }
  };
    ...
    switch (language) {
      ...
      default:
        return {
          title: "Объемный вес",
          description:
            "Некоторые предметы имеют небольшой вес, но отличаются большим объемом. Если объемный вес превышает физический, то тариф на доста��ку определяется в соотве��ствии с объё��ным весом, который рассчитывается по следующей формуле:",
          formula:
            "Объемный вес (кг) = (длин�� (см) х ширина (см) х ����сота (см)) / 6000",
        };
    }
  };
      ...
      case "ru":
        return {
          ...
          rates: [
            ...
            "от 500 кг до 1000 кг – каждый килограмм по 3000 сум",
            "от 1000 кг до 1500 кг – каждый килограмм по 3000 сум",
          ],
        };
      case "uz":
        return {
          ...
          rates: [
            ...
            "500 kg dan 1000 kg gacha — har kg uchun 3 000 so'm",
            "1000 kg dan 1500 kg gacha — har kg uchun 3 000 so'm",
          ],
        };
      case "en":
        return {
          ...
          rates: [
            ...
            "from 500 kg to 1000 kg — 3,000 UZS per kg",
            "from 1000 kg to 1500 kg — 3,000 UZS per kg",
          ],
        };
      default:
        return {
          ...
          rates: [
            "от 1 ��г до 99 кг – каждый килограмм по 5000 сум",
            "от 100 кг до 499 кг – каждый килограмм по 4000 сум",
            "от 500 кг до 1000 кг – каждый килограмм по 3000 сум",
            "от 1000 кг до 1500 кг – каждый килограмм по 3000 сум",
          ],
    ...
    switch (language) {
      case "ru":
        return {
          ...
          notes: [
            ...
            "Если получатель отсутствует по адресу дос��авки или не может принять Отправление по другим, не зависящим от курьера причинам, то каждый последующий приезд курьера тарифицируется как новая отдельная заявка на доставку;",
      ...
      default:
        return {
          title: "ПРИМЕЧАНИЕ:",
          notes: [
            "Каждое Отправление тарифицируется п�� отдельности. В стоимость включена одна попытка доставки Отправления;",
            ...
            "Если Получатель изменил адрес доставки и просит дост��вить по дру��ому адресу, то перенаправление пос��лки по новому адресу, будет тарифицироваться как отдельная заявка на доставку;",
            "Вес одного места Отправления не должен превышать - 30 кг;",
            "Если при вызове курьера Отправитель откажется передавать Отправление к доставке, Заказчик должен оплатить за ложный вызов курьера 25 000 сум без НДС (28 000 с НДС)",
          ],
          effectiveDate: "Тарифы вступ��ют в силу с 1 июля 2025 г.",
        };
    }
  };
  ...
  const getCustomerPickupHeader = () => {
    switch (language) {
      ...
      default:
        return "Забор/доставка от/до к��иента";
    }
  };
  ...
  const getTashkentHeader = () => {
    switch (language) {
      ...
      default:
        return "Забор/доставка по г. Т��шкент";
    }
  };
  ...
  return (
    <div className="space-y-6 w-full max-w-none lg:max-w-[1700px] lg:mx-auto">
      {/* Main Title Section */}
            ...
            <table className="w-full border-collapse min-w-[800px] text-xs">
              <thead>
                <tr className="bg-red-600 text-white">
                  <th className="border border-white px-1 py-1 text-xs font-medium text-center">
                    {getWeightHeader()}
                  </th>
                  <th className="border border-white px-1 py-1 text-xs font-medium text-center whitespace-pre-line">
                    {zoneHeaders.zone0}
                  </th>
                  <th className="border border-white px-1 py-1 text-xs font-medium text-center whitespace-pre-line">
                    {zoneHeaders.zone1}
                ...
                {ratesData.map((row, index) => (
                  ...
                  >
                    ...
                    {row.weight === 1 ? (
                      <td
                        rowSpan={10}
                        className="border border-gray-300 px-1 py-1 text-center align-middle font-medium text-xs"
                      >
                        20,000
                      </td>
                    ) : row.weight <= 10 ? null : (
        ...
        >
          ...
          >
            <thead>
              <tr>
                <th
                  className="border border-gray-300 px-2 py-1 text-xs font-medium text-center bg-white"
            ...
            <tbody>
              {zoneMatrix.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  <td
                    className="border border-gray-300 px-2 py-1 text-center font-medium bg-white text-xs"
                    style={{ minWidth: "100px" }}
                  >
                    {cityNames[rowIndex]}
                  </td>
