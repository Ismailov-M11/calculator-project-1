    ...
    switch (language) {
      case "ru":
        return {
          ...
          cost: "Стоимость: 3% (с учет��м НДС 12%)",
    ...
    switch (language) {
      case "ru":
        return {
          ...
          description:
            "Дополнительный платеж, уплачиваемый Заказчиком Исполните��ю при оглашении стоимости почтового отправления. Расчет размера дополнительного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x объявленная стоимость (сумма).",
        };
      case "uz":
        return {
      ...
      default:
        return {
          ...
          cost: "Стоимость: 1% (без учета НДС 12%)",
          description:
            "Дополнительный платеж, уплачиваемый Заказчи���ом Исполнителю при оглашении стоимости почтового отправления. Расчет размера доп��лнительного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x объявленная стоимость (сумма).",
        };
    }
  };
    ...
    switch (language) {
      ...
      default:
        return {
          title: "Объемный вес",
          description:
            "Некоторые предметы имеют небольшой вес, но отличаются большим объемом. Если объемный вес превышает физический, то тариф на доста��ку определяется в соответствии с объё��ным весом, который рассчитывается по следующей формуле:",
          formula:
            "Объемный вес (кг) = (длина (см) х ширина (см) х ����сота (см)) / 6000",
        };
    }
  };
      ...
      case "ru":
        return {
          ...
          notes: [
            ...
            "Если получатель отсутствует по а��ресу доставки или не может принять Отправление по другим, не зависящим от курьера причинам, то каждый последующий приезд курьера тарифицируется как новая отдельная заявка на доставку;",
      ...
      default:
        return {
          ...
          notes: [
            ...
            "Если Получатель изменил адрес доставки и просит доставить по дру��ому адресу, то перенаправление посылки по новому адресу, будет тарифицироваться как отдельная заявка на доставку;",
            "Вес одного места Отправления не должен превышать - 30 кг;",
            "Если при вызове курьера Отправитель откажется передавать Отправление к доставке, Заказчик должен оплатить за ложный вызов курьера 25 000 сум без НДС (28 000 с НДС)",
          ],
