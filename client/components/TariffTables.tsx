...
export function TariffTables() {
  ...
  const getMainSubtitle = () => {
    switch (language) {
      case "ru":
        return "Стоимо��ть услуг курьерской службы в Республике Узбекистан (без учёта НДС)";
      case "uz":
    ...
    switch (language) {
      ...
      default:
        return {
          ...
          cost: "Стоимость: 1% (без учета НДС 12%)",
          description:
            "Дополнительный платеж, уплачиваемый Заказчи�����м Исполнителю при оглашении стоимости почтового отправления. Расчет размера доп��лнительного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x объявленная стоимость (сумма).",
        };
    }
  };
    ...
    switch (language) {
      case "ru":
        return {
          ...
          formula:
            "Объемный вес (кг) = (длина (с��) х ширина (см) х высота (см)) / 6000",
        };
      case "uz":
        return {
    ...
    switch (language) {
      case "ru":
        return {
          title: "ПРИМЕЧАНИЕ:",
          notes: [
            "Каждое Отправление тарифицируется по отдельности. В стоимо��ть включена одна попытка доставки Отправления;",
          ...
          effectiveDate: "Тарифы вс��упают в силу с 1 июля 2025 г.",
        };
      case "uz":
        return {
          title: "IZOH:",
      ...
      default:
        return {
          ...
          notes: [
            ...
            "Если получатель отсутствует по адресу доставки или не может принять Отправление по другим, не зависящим от курьера причинам, то каждый последующий приезд курьера тарифицируется как новая отдельная заявка на доставку;",
            "Если Получатель изменил адрес доставки и просит доставить по дру��ому адресу, то перенаправление пос��лки по новому адресу, будет тарифицироваться как отдельная заявка на доставку;",
            "Вес одного места Отправления не должен превышать - 30 кг;",
            "Если при вызове курьера Отправитель откажется передавать Отправление к доставке, Заказчик должен оплатить за ложный вызов курьера 25 000 сум без НДС (28 000 с НДС)",
          ],
